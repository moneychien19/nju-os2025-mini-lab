...
- 如何每 100ms 輸出一次？
  - 在每次讀完一行後，用 `clock_gettime(CLOCK_MONOTONIC, ...)` 取得當前時間，計算距離上次輸出過了幾毫秒，超過 100ms 就輸出一次實時統計並更新上次輸出的時間
  - 注意這個方法並非真正的定時器，只有在有新的 strace 輸出時才會觸發檢查；對於 syscall 頻繁的程式（如 `find`）效果正常，對於 syscall 稀少的程式（如 `sleep`）則不會定時輸出